...
export const DashboardManagement = () => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        const response = await axios.get(`/api/orders/revenue/${year}`,{
          headers: {
        ...
        const { orderCustomerRevenue, orderRevenue } = response.data.data; // Assuming your API returns data in this format
        
        
        setSpendingData(orderRevenue);
        setSalesData(orderCustomerRevenue.data.data);
      } catch (error) {
  ...
  const spendingVsSalesData = {
    ...
    datasets: [
      {
        label: 'Amount Spent on Ordering',
        data: spendingData.slice(startMonthIndex, startMonthIndex + 6).map(item => item.revenue),
        fill: false,
        backgroundColor: 'rgba(75,192,192,0.4)',
        borderColor: 'rgba(75,192,192,1)',
      },
      {
        label: 'Amount of Product Sold',
        data: salesData.slice(startMonthIndex, startMonthIndex + 6).map(item => item.revenue),
        fill: false,
        backgroundColor: 'rgba(153,102,255,0.4)',
        borderColor: 'rgba(153,102,255,1)',
      },
  ...
  const getRevenueForMonth = (monthIndex) => {
    const revenueEntry = salesData.find(entry => entry.month - 1 === monthIndex);
    return revenueEntry ? revenueEntry.revenue : 0;
  };
    ...
    datasets: [
      {
        ...
        data: salesData.slice(startMonthIndex, startMonthIndex + 6).map(item => item.revenue),
        // data: monthsToShow.map((month, index) => getRevenueForMonth(startMonthIndex + index)),
        fill: false,
        backgroundColor: 'rgba(153,102,255,0.4)',
        borderColor: 'rgba(153,102,255,1)',
        borderWidth: 1,
